...
outputList.append('data/output/sampleStats.filtered.csv')
...
#Eukaryota / Crassphage
if config['use_eukaryota_verification']:
    outputList+=['data/output/eukaryotes/summary.csv']
        ...
        if sig['signalid'] == s['file'] or s['mode'] == 'fastq':
            if not sig['signalid'] in missingSignals:
                ...
                if config['use_assembly']:
                    'data/output/assembly/'+s['patientid']+'_'+str(s['time'])+'/prokka/annotation.gff',
...
for row in samples.itertuples():
    if row.mode == 'fastq':
        ...
        inputfile = 'data/input/reads/'+row.file+'.fastq'
        auxdir = 'data/auxiliary/samples/'+row.patientid+'_'+str(row.time)
        auxfile = auxdir+'/reads.fastq'

        #Sanity check
        if not os.path.exists(inputfile):
            print("Readfile {}.fastq could not be found in data/input/reads but was specified in the samples.tsv table, aborting ...!".format(row.file))
            sys.exit(-1)
        else:
            if not os.path.exists(auxfile):
                cmd = 'mkdir -p '+auxdir+' && ln -srf '+inputfile+' '+auxfile
                print("Sample: {} {} starts the pipeline with a fastq file, creating symlink ...".format(row.patientid,row.time))
                shell(cmd)
